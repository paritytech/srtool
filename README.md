...
-   `paritytech/srtool:1.92.0-0.18.4`

-   `paritytech/srtool:1.92.0`

The tags not mentioning the build version always point to the latest one. In the example above, `paritytech/srtool:1.92.0` is the same image than `paritytech/srtool:1.92.0-0.18.4`.
...
Creating an alias helps hiding the docker complexity behind one simple command. We will see more powerful options but this one is simple enough.
        ...
        export RUSTC_VERSION=1.92.0; export PACKAGE=kusama-runtime; alias srtool='docker run --rm -it -e PACKAGE=$PACKAGE -v $PWD:/build -v $TMPDIR/cargo:/cargo-home paritytech/srtool:$RUSTC_VERSION'
...
will output something that looks like this:
        ...
        🧰 Substrate Runtime Toolbox - srtool v0.18.4 🧰
                  - by Chevdor -
        🏗  Building polkadot-runtime as release using rustc 1.92.0
...
The `info` and `version` scripts pass any arguments you pass to the script to `jq`. So you can play with `c` (compact), `-M` (monochrome), `-C` color output. For instance `docker run --rm -it -v $PWD:/build chevdor/srtool:1.92.0 info -cM` shows a monochrome output on a single line.
...
`srtool` includes a command that helps finding runtimes in a repo.
        ...
        podman run --rm -it \
            -v $REPO:/build \
            `paritytech/srtool:1.92.0-0.18.4` scan
...
So say you want to build a builder for rustc 1.92.0:

        RUSTC_VERSION=1.92.0 && docker build --build-arg RUSTC_VERSION=$RUSTC_VERSION -t paritytech/srtool:$RUSTC_VERSION .
