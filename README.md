...
-   `paritytech/srtool:1.57.0-0.9.16`

-   `paritytech/srtool:1.57.0-0.9.19`

-   `paritytech/srtool:1.57.0`

The tags not mentioning the build version always point to the latest one. In the example above, `paritytech/srtool:1.57.0` is the same image than `paritytech/srtool:1.57.0-0.9.19`.
...
The last version hosted on Gitlab has been built using Rust Stable 1.57.0. It is tagged as v0.9.19 and there is no plan on updating the Gitlab repository further. New versions will be available from [this repository](https://www.github.com/paritytech/srtool) only. The functionalities remain the same so you can (and should!) simply swap `chevdor/srtool` for `paritytech/srtool` in your workflows. The [srtool-actions](https://github.com/chevdor/srtool-actions) will remain available as `chevdor/srtool-actions@<version>` and will be updated to point at the paritytech image.
...
Creating an alias helps hiding the docker complexity behind one simple command. We will see more powerful options but this one is simple enough.
        ...
        export RUSTC_VERSION=1.57.0; export PACKAGE=kusama-runtime; alias srtool='docker run --rm -it -e PACKAGE=$PACKAGE -v $PWD:/build -v $TMPDIR/cargo:/cargo-home paritytech/srtool:$RUSTC_VERSION'
...
will output something that looks like this:
        ...
        🧰 Substrate Runtime Toolbox - srtool v0.9.19 🧰
                  - by Chevdor -
        🏗  Building polkadot-runtime as release using rustc 1.57.0
