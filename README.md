...
-   `paritytech/srtool:1.84.0-0.17.0`

-   `paritytech/srtool:1.84.0`

The tags not mentioning the build version always point to the latest one. In the example above, `paritytech/srtool:1.84.0` is the same image than `paritytech/srtool:1.84.0-0.17.0`.
...
`srtool` includes a command that helps finding runtimes in a repo.
        ...
        podman run --rm -it \
            -v $REPO:/build \
            `paritytech/srtool:1.84.0-0.17.0` scan
