...
-   `paritytech/srtool:1.59.0-0.9.16`

-   `paritytech/srtool:1.59.0-0.9.20`

-   `paritytech/srtool:1.59.0`

The tags not mentioning the build version always point to the latest one. In the example above, `paritytech/srtool:1.59.0` is the same image than `paritytech/srtool:1.59.0-0.9.20`.
...
Creating an alias helps hiding the docker complexity behind one simple command. We will see more powerful options but this one is simple enough.
        ...
        export RUSTC_VERSION=1.59.0; export PACKAGE=kusama-runtime; alias srtool='docker run --rm -it -e PACKAGE=$PACKAGE -v $PWD:/build -v $TMPDIR/cargo:/cargo-home paritytech/srtool:$RUSTC_VERSION'
...
will output something that looks like this:
        ...
        🧰 Substrate Runtime Toolbox - srtool v0.9.20 🧰
                  - by Chevdor -
        🏗  Building polkadot-runtime as release using rustc 1.59.0
...
So say you want to build a builder for rustc 1.59.0:

        RUSTC_VERSION=1.59.0 && docker build --build-arg RUSTC_VERSION=$RUSTC_VERSION -t paritytech/srtool:$RUSTC_VERSION .
